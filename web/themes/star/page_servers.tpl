				...
				<div class="table-responsive" id="server_group">
					...
					<table class="table table-bordered table-striped table-hover tbl-sm">
						...
						<tbody>
							...
							<tr style="cursor:pointer;" data-toggle="collapse" data-target="#expand_{$server.sid}" aria-expanded="{if isset($smarty.get.s) && $smarty.get.s==$index}true{else}false{/if}" aria-controls="collapseExample" {if !$IN_SERVERS_PAGE} onclick="{$server.evOnClick}"{/if}>
